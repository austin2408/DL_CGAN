...
parser.add_argument('--store', default='/home/austin/nctu_hw/DL/DL_hw7/Task1/GAN/model/', type=str)
...
test_dataloader = DataLoader(test_datasets, batch_size = len(test_datasets), shuffle = False)

new_datasets = ICLEVRLoader(args.data, cond=True, mode='new')
new_dataloader = DataLoader(new_datasets, batch_size = len(new_datasets), shuffle = False)
...
for epoch in range(args.epochs):
    ...
    wandb.log({"Score new": score_new})

    print('Epoch : ',epoch+1,' D loss : ',sum(loss_d)/len(loss_d),' G loss : ',sum(loss_g)/len(loss_g), ' Score : ', round(score,2), ' Score new : ', round(score_new,2))

    if (score>=0.65):
        save_image(gen_img, os.path.join(args.store, f'2epoch{epoch+1}.png'), nrow=8, normalize=True)
        torch.save(netD.state_dict(), args.store+'Discriminator_'+str(epoch+1)+'_'+str(score)+'_'+str(score_new)+'.pth')
        torch.save(netG.state_dict(), args.store+'Generator_'+str(epoch+1)+'_'+str(score)+'_'+str(score_new)+'.pth')
